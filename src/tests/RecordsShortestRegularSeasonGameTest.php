...
class RecordsShortestRegularSeasonGameTest extends \App\Infrastructure\Test\TestCaseCore
{
    ...
    public function test_record()
    {
        ...
        $entries = $records[\App\Listeners\Records\ShortestRegularSeasonGame::BASE_KEY]['entries'];
    ...
    public function test_multi_game()
    {
        ...
        $records = $this->recordStore->getRecords();
        $this->assertEquals(2, count($records[\App\Listeners\Records\ShortestRegularSeasonGame::BASE_KEY]['entries']));
    ...
    public function test_edit_game()
    {
        ...
        $records = $this->recordStore->getRecords();
        $this->assertEquals(1, count($records[\App\Listeners\Records\ShortestRegularSeasonGame::BASE_KEY]['entries']));

    }

    public function test_multi_game_for_shorter_game()
    {
        $this->generateGame('9:00 AM', '9:30 AM', 1);
        $shortGameId = $this->generateGame('9:00 AM', '9:10 AM', 2);
        $records = $this->recordStore->getRecords();

        $entries = $records[\App\Listeners\Records\ShortestRegularSeasonGame::BASE_KEY]['entries'];
        $this->assertEquals(1, count($entries));
            $this->assertEquals("[season=1]: [gameId=".$shortGameId." gameNumber=2] - 10 minutes. 12-1 for Colored Team. Goalies: ([playerId=".$this->chrisLee.'] white vs. [playerId='.$this->davidR.'] colored)', $entries[0]);
    }
