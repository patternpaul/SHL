...
class MostPointsInARegularSeason extends Listener
{
    private $redis;
    private $recordStore;

    CONST BASE_KEY = 'mostPointsInARegularSeason';
    private $baseKey = MostPointsInARegularSeason::BASE_KEY;
    ...
    private function storeRecord()
    {
        ...
        foreach ($recordHolders as $recordHolder) {
            ...
            $recordEntries[] = [
                'entry' =>   RecordStore::seasonEncode($playerData[1])." ".RecordStore::playerEncode($playerData[0])." ".$currentMost." Points.",
                'playerKeys' => [$playerData[0]]
            ];
        }
        $this->recordStore->setRecord($this->baseKey, 'Most Points Overall By A Player In One Season', $recordEntries);
    }
