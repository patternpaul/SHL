 ...
 * Sort Functions
 */
 if(typeof(filterAllLang) === 'undefined')
 {
 	filterAllLang = "filter: All";
 }
	...
	table.filter = function(o,filters,args) {
		...
		if (!filters) {
			...
			if (filters.nodeName=="SELECT" && filters.type=="select-one" && filters.selectedIndex>-1) {
				filters={ 'filter':filters.options[filters.selectedIndex].value.toUpperCase() };
			}
			// Also allow for a regular input
			if (filters.nodeName=="INPUT" && filters.type=="text") {
				filters={ 'filter':"/^"+filters.value.toUpperCase()+"/" };
			}
					...
					for (col in tdata.filters) {
						if (!hideRow) {
							...
							if (filter && col<cellsLength) {
								var val = this.getCellValue(cells[col]).toUpperCase();
